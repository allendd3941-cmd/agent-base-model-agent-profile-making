...
// 若你把本檔放到 api_post_module.gaml 同一資料夾，可改成：import "api_post_module.gaml"
import "C:/Users/allen/Desktop/master_1.2/小實習/LLM_abm_model/gama_moudle/api_post_module.gaml"

global {
    // === 圖資與座標設定 ===
    // ROADLINK 是 agent 可行走道路；座標為 TWD97 TM Taiwan，單位 meter。
    file shape_file_roads <- file("C:/Users/allen/Downloads/20260515GAMA/ROADLINK.shp");

    // TOWN_MOI 是鄉鎮市區界線；只保留 COUNTYNAME = "臺南市"。
    file shape_file_towns <- file("C:/Users/allen/Desktop/圖資資料庫/鄉(鎮、市、區)界線1140318/TOWN_MOI_1140318.shp");
    ...
    // TODO: 固定終點鄉鎮市區。請改成任一臺南市 TOWNNAME，例如 "東區"、"安南區"、"歸仁區"。
    ...
    string agent_memory_path <- "C:/Users/allen/Downloads/20260515GAMA/agent_memory.csv";
    string road_flow_path <- "C:/Users/allen/Downloads/20260515GAMA/road_flow.csv";

    // 每個 step 送給 Python 前重建的 payload。
    list<map> payload_agents_data <- [];
    list<map> payload_roads_data <- [];
    ...
    list<string> exportable_environment_variables <- [
        ...
        "simulation.step_minutes",
        "simulation.elapsed_minutes",
        "simulation.max_steps",
        "spatial.crs_note",
        "spatial.world_envelope",
        "spatial.current_point",
        "spatial.current_town",
        "spatial.destination_town",
        "road.road_id",
        "road.NAME",
        "road.highway",
        "road.highway_ty",
        ...
        "agent.route_status",
        "neighborhood.nearby_road_ids",
        "neighborhood.nearby_agent_count",
        "neighborhood.congestion_proxy",
        "neighborhood.available_road_choices",
    ...
    init {
        ...
        try {
            ...
            write "[WARN] TOWN_MOI CRS transform failed; if towns and roads do not overlap, reproject TOWN_MOI before loading.";
        }
        ...
        save [
            ...
            "active_mode", "speed_kmh", "distance_moved_m",
        ...
        create vehicle number: nb_agents {
            ...
            target <- choose_road_point_in_town(destination_town);
    ...
    action send_initial_request {
        ...
        map init_payload <- [
            ...
            "crs_note"::"ROADLINK=TWD97 TM Taiwan meter; TOWN_MOI=GCS_TWD97[2020], transformed with to_GAMA_CRS when possible."
        ];
    ...
    action send_step_request {
        ...
        payload_roads_data <- [];
        ask road {
            payload_roads_data << build_road_payload();
        }
    ...
    point choose_road_point_in_town(string requested_town_name) {
        ...
        if (selected_town != nil) {
            list<road> roads_in_town <- road overlapping selected_town;
            ...
            if (nearest_road != nil) {
                write "[WARN] No overlapping ROADLINK in " + selected_town.town_name + "; use nearest road instead.";
                return any_location_in(nearest_road);
            }

            write "[WARN] No ROADLINK found near " + selected_town.town_name + "; use town centroid.";
    ...
    map extract_active_mode_from_body(string body) {
        ...
        try {
            ...
            if (parsed is map) {
                ...
                } else if (reply.keys contains "vehicle_type") {
                    mode_payload <- ["mode_name"::string(reply["vehicle_type"])];
                }
            }
        } catch {
            mode_payload <- [];
            ...
            if (parsed is map) {
                ...
                if (!empty(rows)) {
                    ...
                    loop row over: rows {
                        ...
                        if (target_agent != nil) {
                            ask target_agent {
                                do place_from_origin(response_origin, response_mode);
                            }
                        }
                    }
                }
    ...
    reflex send_payload_to_api when: enable_api_post and init_request_sent and cycle < max_steps {
        // 每個 step 發送一次完整狀態；payload 保留 cycle/vehicles 相容既有 Python。
        do send_step_request;
    }

    reflex record_road_flow {
        // 每步紀錄所有道路 flow，並同步成 payload_roads_data。
        ask road {
            save [
                cycle, road_id, road_name, highway, highway_type,
                current_flow, capacity, congestion_proxy
...
species road {
    ...
    reflex update_flow {
        ...
        congestion_proxy <- (capacity > 0.0) ? min([1.0, current_flow / capacity]) : min([1.0, current_flow / 10.0]);
        weight <- max([shape.perimeter, 1.0]) * (1.0 + current_flow * 2.0);
    }

    aspect default {
        rgb color_by_flow <- (current_flow > 8) ? #red : ((current_flow > 3) ? #orange : #black);
...
species vehicle skills: [moving] {
    ...
    float desired_speed <- 40.0;
    float speed_car_preference <- 45.0;
    float speed_moto_preference <- 35.0;
    list<string> road_type_preference <- ["primary", "secondary", "tertiary", "residential"];
    float route_randomness <- 0.15;
    float comfort_weight <- 0.20;
    float time_weight <- 0.45;
    float distance_weight <- 0.25;
    float capacity_weight <- 0.10;
    ...
    action place_from_origin(string requested_origin, map mode_payload) {
    ...
    town current_town {
        ...
        town current_area <- town first_with (self.location inside each.shape);
        if (current_area = nil) {
    ...
    map build_environment_payload {
        ...
        list<string> nearby_road_ids <- [];
        loop nearby_rd over: (road at_distance perception_radius) {
            nearby_road_ids << nearby_rd.road_id;
        }

        list<vehicle> nearby_agents <- (vehicle at_distance perception_radius) - self;
        map road_payload <- (current_rd = nil) ? [] : current_rd.build_road_payload();
        ...
        return [
            ...
            "spatial"::[
                "crs_note"::"ROADLINK=TWD97 TM Taiwan meter; TOWN_MOI=GCS_TWD97[2020], transformed with to_GAMA_CRS when possible.",
                "world_envelope"::string(shape),
                "current_point"::string(location),
    ...
    map build_memory_entry {
        ...
        return [
            ...
            "current_road_name"::(current_rd = nil ? "" : current_rd.road_name),
            "current_road_type"::(current_rd = nil ? "" : current_rd.highway_type),
            "active_mode"::build_active_mode_payload(),
            "selected_action"::selected_action,
            "speed_used_kmh"::speed * 3.6,
            "distance_moved_m"::distance_moved_last_step,
            "distance_to_destination_m"::(target = nil ? 0.0 : location distance_to target),
            "nearby_agent_count"::length(nearby_agents),
            "api_request_status"::api_status,
            "api_response_summary"::last_api_response_summary,
    ...
    reflex perceive_environment when: !waiting_for_origin {
        ...
        if (current_rd != nil) {
            float road_limit <- (mode_name contains "moto" or mode_name contains "機車") ? current_rd.speed_moto : current_rd.speed_car;
            if (road_limit > 0.0 and base_speed > road_limit) {
                base_speed <- road_limit;
            }
            if (road_limit <= 0.0 and base_speed > 40.0) {
                base_speed <- 40.0;
            }
        }
        ...
        speed <- (is_crowded ? base_speed * 0.55 : base_speed) #km / #h;
    }
    ...
    reflex move when: !waiting_for_origin and route_status != "arrived" {
        ...
        if (target != nil and location distance_to target < 30 #m) {
    ...
    aspect default {
        ...
        if (mode_name contains "car" or mode_name contains "汽車" or mode_name contains "開車") {
            draw triangle(450) color: agent_color rotate: heading + 90;
        } else {
            draw circle(220) color: agent_color;
        }
    }
}

experiment Traffic_Simulation type: gui {
    // GUI 參數：不用改程式也能調整 API、agent 數量與固定終點。
    parameter "Enable Python API POST" var: enable_api_post;
    parameter "Number of agents" var: nb_agents min: 1 max: 200;
    parameter "Fixed destination town" var: destination_town_name;
